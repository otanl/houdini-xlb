...
`/obj/houdini_xlb`内のbuilding Box SOPにはフレーム1、41、81、120の
キーフレームがあります。12 fps・120フレームなので約10秒かけてゆっくり移動します。
...
READMEのGIFは120フレームから13案を等間隔にサンプリングし、13案すべてを
実際にdraft解析します。風速場の画像補間はしていないため、建物移動とCFD結果が
同じ刻みで進みます。中間PNGとXLBキャッシュは`artifacts/readme-demo/`に保存され、
Gitには含まれません。
