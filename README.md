...
同梱のdraft解析では、快適域内の広場セルが`39.6% → 83.5%`、通風帯が
`95.2%`維持となり、全16候補中の大域最良解は評価11で見つかります。タイムラインは
評価1、3、5、11と探索完了16におけるbest-so-farだけを再生します。快適域はUIと
最適化ループを説明するための仮設定であり、風環境の認証・適否判定にはquality解析、
複数風向・季節条件、実測または別ソルバーによる検証が必要です。
